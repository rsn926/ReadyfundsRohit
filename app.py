...
with tab1:
    ...
    with col1:
        ...
        fig = px.pie(
            ...
            names=industry_counts.index.tolist(),
            hole=0.4
        )
    ...
    with col2:
        ...
        # Convert to native Python types
        fig = px.bar(
            x=adoption_counts.index.tolist(),
            y=adoption_counts.values.tolist(),
            color=adoption_counts.values.tolist(),
...
with tab2:
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            orientation='h',
            color=top_features['importance'].tolist(),
...
with tab3:
    ...
    with col1:
        ...
        # Convert to native types
        fig = px.pie(
            values=cluster_sizes.values.tolist(),
            names=[f"Segment {int(i)}" for i in cluster_sizes.index],
            hole=0.4
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        for i in range(5):
            ...
            if len(cluster_data) > 0:
                ...
                adoption_by_cluster.append({'Segment': f'Segment {i}', 'Adoption Rate': float(adoption_rate)})
        
        adoption_df = pd.DataFrame(adoption_by_cluster)
        # Convert to native types
        fig = px.bar(
            adoption_df,
            x=adoption_df['Segment'].tolist(),
            y=adoption_df['Adoption Rate'].tolist(),
            color=adoption_df['Adoption Rate'].tolist(),
    ...
    if models['arm_rules'] is not None and len(models['arm_rules']) > 0:
        ...
        with col1:
            # Convert data to native types for Plotly - remove problematic hover_data
            arm_plot_data = models['arm_rules'].head(50).copy()
            fig = px.scatter(
                arm_plot_data,
                x=arm_plot_data['support'].tolist(),
                y=arm_plot_data['confidence'].tolist(),
                size=arm_plot_data['lift'].tolist(),
                color=arm_plot_data['lift'].tolist()
            )
        ...
        with col2:
            # Convert to native types
            lift_values = models['arm_rules']['lift'].tolist()
            fig = px.histogram(x=lift_values, nbins=30)
            fig.update_layout(title="Distribution of Lift Values", xaxis_title="Lift", yaxis_title="Count")
            st.plotly_chart(fig, use_container_width=True)
...
with tab5:
    ...
    with col1:
        ...
        fig = px.pie(
            ...
            names=risk_counts.index.tolist(),
            color_discrete_map={
    ...
    with col2:
        ...
        # Convert to native types
        fig = px.bar(
            x=default_counts.index.tolist(),
            y=default_counts.values.tolist(),
            color=default_counts.index.tolist(),
            color_discrete_map={
                'Never': '#2ecc71',
                'Occasionally': '#f39c12',
                'Frequently': '#e74c3c'
        ...
        fig.update_layout(showlegend=False, xaxis_title="Default History", yaxis_title="Count")
        st.plotly_chart(fig, use_container_width=True)
    ...
    # Convert to native types
    fig = px.bar(
        x=avg_pricing.index.tolist(),
        y=avg_pricing.values.tolist(),
        color=avg_pricing.values.tolist(),
        color_continuous_scale='RdYlGn_r'
    )
    fig.update_layout(xaxis_title="Risk Category", yaxis_title="Monthly Rate (%)", showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
