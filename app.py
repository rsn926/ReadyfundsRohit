...
def engineer_features(df):
    ...
    df_processed['Digital_Readiness_Score'] = (
        (df_processed['Comfort with App'].map({'Yes': 50, 'No': 0})) +
        (df_processed['Willing to Share Data'].map({'Yes': 30, 'No': 0})) +
        (df_processed['Digital Transactions %'].map({
            '<20%': 5, '20–50%': 10, '50–80%': 15, '80%+': 20
        }))
    )
    ...
    df_processed['Credit_Health_Score'] = (
        df_processed['Default History'].map(default_weights) +
        df_processed['Revenue Stability'].map(stability_weights) +
        df_processed['GST Filing'].map(gst_weights)
    )
    ...
    df_processed['Business_Maturity_Index'] = (
        df_processed['Years in Operation'].map(years_weights) * 0.6 +
        df_processed['Monthly Revenue'].map(revenue_weights) * 0.4
    )
    
    # Binary encodings
    df_processed['Uses_Credit_Binary'] = df_processed['Uses Credit Currently'].map({'Yes': 1, 'No': 0})
    df_processed['GST_Filed_Binary'] = df_processed['GST Filing'].map({'Yes': 1, 'No': 0})
